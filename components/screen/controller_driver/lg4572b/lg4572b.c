...
static const char *TAG = "lcd lg4572b";
...
#define LCD_NAME "LG4572B"
#define LCD_BPP  16

/** 
 * LG4572B can select different resolution, but I can't find the way
 */
#define LG4572B_RESOLUTION_HOR 480
#define LG4572B_RESOLUTION_VER 800

#define LG4572B_CASET   0x2A
#define LG4572B_RASET   0x2B
#define LG4572B_RAMWR   0x2C
#define LG4572B_MADCTL  0x36
...
scr_driver_t lcd_lg4572b_default_driver = {
    .init = lcd_lg4572b_init,
    .deinit = lcd_lg4572b_deinit,
    .set_direction = lcd_lg4572b_set_rotation,
    .set_window = lcd_lg4572b_set_window,
    .write_ram_data = lcd_lg4572b_write_ram_data,
    .draw_pixel = lcd_lg4572b_draw_pixel,
    .draw_bitmap = lcd_lg4572b_draw_bitmap,
    .get_info = lcd_lg4572b_get_info,
};


static void lcd_lg4572b_init_reg(void);

esp_err_t lcd_lg4572b_init(const scr_controller_config_t *lcd_conf)
{
    LCD_CHECK(lcd_conf->width <= LG4572B_RESOLUTION_HOR, "Width greater than maximum", ESP_ERR_INVALID_ARG);
    LCD_CHECK(lcd_conf->height <= LG4572B_RESOLUTION_VER, "Height greater than maximum", ESP_ERR_INVALID_ARG);
    ...
    ret = lcd_lg4572b_set_rotation(lcd_conf->rotate);
    LCD_CHECK(ESP_OK == ret, "set rotation failed", ESP_FAIL);

    return ESP_OK;
}

esp_err_t lcd_lg4572b_deinit(void)
{
    memset(&g_lcd_handle, 0, sizeof(scr_handle_t));
    return ESP_OK;
}

esp_err_t lcd_lg4572b_set_rotation(scr_dir_t dir)
{
    esp_err_t ret;
    uint8_t reg_data = 0;
    reg_data &= ~MADCTL_RGB;
    ...
    ret = LCD_WRITE_REG(LG4572B_MADCTL, reg_data);
    LCD_CHECK(ESP_OK == ret, "Set screen rotate failed", ESP_FAIL);
    g_lcd_handle.dir = dir;
    return ESP_OK;
}

esp_err_t lcd_lg4572b_get_info(scr_info_t *info)
{
...
esp_err_t lcd_lg4572b_set_window(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
    ...
    scr_utility_apply_offset(&g_lcd_handle, LG4572B_RESOLUTION_HOR, LG4572B_RESOLUTION_VER, &x0, &y0, &x1, &y1);

    ret |= LCD_WRITE_REG(LG4572B_CASET, x0 >> 8);
    ret |= LCD_WRITE_REG(LG4572B_CASET + 1, x0 & 0xff);
    ret |= LCD_WRITE_REG(LG4572B_CASET + 2, x1 >> 8);
    ret |= LCD_WRITE_REG(LG4572B_CASET + 3, x1 & 0xff);
    ret |= LCD_WRITE_REG(LG4572B_RASET, y0 >> 8);
    ret |= LCD_WRITE_REG(LG4572B_RASET + 1, y0 & 0xff);
    ret |= LCD_WRITE_REG(LG4572B_RASET + 2, y1 >> 8);
    ret |= LCD_WRITE_REG(LG4572B_RASET + 3, y1 & 0xff);

    ret |= LCD_WRITE_CMD(LG4572B_RAMWR);
    LCD_CHECK(ESP_OK == ret, "Set window failed", ESP_FAIL);
    return ESP_OK;
}

esp_err_t lcd_lg4572b_write_ram_data(uint16_t color)
{
...
esp_err_t lcd_lg4572b_draw_pixel(uint16_t x, uint16_t y, uint16_t color)
{
    esp_err_t ret;
    ret = lcd_lg4572b_set_window(x, y, x, y);
    if (ESP_OK != ret) {
        return ESP_FAIL;
    }
    return lcd_lg4572b_write_ram_data(color);
}

esp_err_t lcd_lg4572b_draw_bitmap(uint16_t x, uint16_t y, uint16_t w, uint16_t h, uint16_t *bitmap)
{
    esp_err_t ret;
    LCD_CHECK(NULL != bitmap, "bitmap pointer invalid", ESP_ERR_INVALID_ARG);

    LCD_IFACE_ACQUIRE();
    ret = lcd_lg4572b_set_window(x, y, x + w - 1, y + h - 1);
    if (ESP_OK != ret) {
        return ESP_FAIL;
    }
...
static void lcd_lg4572b_init_reg(void)
{
    ...
    LCD_WRITE_CMD(0xC1); // Power Control 1
    LCD_WRITE_DATA(0x00);
    ...
    LCD_WRITE_CMD(0x13); //Exit Sleep (Normal Mode)
