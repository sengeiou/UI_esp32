...
esp_err_t xpt2046_init(const touch_panel_config_t *config)
{
    ...
    #if CONFIG_TOUCH_PANEL_INTERRUPT_ENABLE == 1
    if (config->pin_num_int >= 0)
    {
        gpio_config_t irq_config = {
            .pin_bit_mask = BIT64(config->pin_num_int),
...
xpt2046_touch_detect_t xpt2048_is_touch_detected(void)
{
    /**
    ...
    #if CONFIG_TOUCH_PANEL_INTERRUPT_ENABLE == 1
    uint8_t irq = gpio_get_level(g_dev.io_irq);
    ...
    #else
    int16_t z1 = ((int16_t)xpt2046_readdata(g_dev.spi_dev, XPT2046_TOUCH_CMD_Z1)) >> 3;
    int16_t z2 = ((int16_t)xpt2046_readdata(g_dev.spi_dev, XPT2046_TOUCH_CMD_Z2)) >> 3;
...
static void xpt2046_corr(int16_t* x, int16_t* y)
{
    ...
    (*x) = (uint32_t)((uint32_t)(*x) * g_dev.width) /
           (XPT2046_X_MAX - XPT2046_X_MIN);

    (*y) = (uint32_t)((uint32_t)(*y) * g_dev.height) /
...
esp_err_t xpt2046_sample(touch_panel_points_t *info)
{
    ...
    int state = xpt2048_is_touch_detected();
    if (0 == state) {
        return ESP_OK;
    }
...
{
    return touch_calibration_run(screen, xpt2048_is_touch_detected, xpt2046_get_rawdata, recalibrate);
}
